...
export default function CreateDietPlan() {
  ...
  const generateAIPlan = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Show a subtle notification if fallback was used (optional)
        if (response.data.isFallback && response.data.message) {
          // You could show a toast notification here instead of console
  ...
  const generatePDF = () => {
    try {
      ...
      doc.text("MealWell - Your Personalized Diet Plan", pageWidth / 2, 20, {
        align: "center",
      });
      ...
      if (user) {
        ...
        doc.text(`Date: ${new Date().toLocaleDateString()}`, pageWidth - 14, 35, {
          align: "right",
        });
      }
      ...
      doc.setFontSize(14);
      doc.setTextColor(0, 0, 0);
      const startY = user ? 50 : 40;
      doc.text("Health Profile", 14, startY);
      ...
      const tipsData = generatedPlan.tips && Array.isArray(generatedPlan.tips)
      ...
      const fileName = `MealWell-Diet-Plan-${new Date().toISOString().split("T")[0]}.pdf`;
      doc.save(fileName);
    } catch (error) {
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button
            onClick={generatePDF}
            className="px-8 py-4 border-2 border-emerald-500 text-emerald-600 rounded-xl font-bold hover:bg-emerald-50 transition-all flex items-center justify-center gap-2"
          >
