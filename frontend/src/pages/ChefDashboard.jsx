...
export default function ChefDashboard() {
  ...
  const downloadPDF = () => {
    try {
      ...
      // Add orders table
      doc.text("Detailed Orders", 14, finalY);

      const tableData = filteredOrders.map((order) => {
        const itemsList = order.items?.map((i) => `${i.name} (x${i.quantity})`).join(", ") || "N/A";
        return [
      ...
      if (tableData.length > 0) {
        ...
        doc.setFontSize(12);
        doc.text("No orders to display", 14, finalY + 15);
      }
